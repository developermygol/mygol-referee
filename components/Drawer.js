        ...
        return (
            <TouchableOpacity style={style.Item} onPress={this.handlePress} activeOpacity={0.2} >
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    {p.icon ? <FontAwesome name={p.icon} size={22} style={style.ItemIcon} color={gColors.text1} /> : null }
...
class Drawer extends Component {
    handleLogout = () => {
        toast.confirm(Localize('Confirm'), Localize('Logout?'))
            .then((res) => {
                if (res !== 'Yes') return;
                const p = this.props;
    ...
    render() {
        ...
        return (
            <View style={style.View}>
                ...
                <DrawerItem title='PersonalDataScreenTitle' navigate='PersonalDataScreen' icon='id-card' navigation={p.navigation} params={{ needsPassword: false }} />
                <DrawerItem title='TermsAndConditions' navigate='TermsAndConditions' icon='file' navigation={p.navigation} />
