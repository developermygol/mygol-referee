    ...
    static confirm(title, msg) {
        return new Promise((resolve, reject) => {
            Alert.alert(title, msg, [ 
                { text: Localize('No'),  onPress: () => { resolve('No') }, style: 'cancel' },
                { text: Localize('Yes'), onPress: () => { resolve('Yes') } },
            ]);
        })
    }
}
...
export const uploadImage = async (target, uri, idUser, uploadType, secure = false) => {
    ...
    try
    {
        ...
        if (err.response && err.response.status === 413)
            toast.error(Localize('Error.Upload.413'));
        else
            toast.error(Localize("Error.UploadingImage") + ' ' + err);
